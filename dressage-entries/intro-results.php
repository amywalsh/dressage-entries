...
<?php
		...
		$intro_entries_sql = "SELECT entries.ID, riders.ID, classes.ID, tests.ID 
				...
				ON tests.ID = classes.TestID WHERE tests.ID IN (1, 2, 3) AND riders.Level = 'Open' AND entries.ShowID = 6";
	...
	<table id="table-wrap" style="width:100%;" border="1">
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name 
				...
				AND entries.Status <> 'Scratch' AND tests.ID = 1 AND riders.Level = 'Open'";
		...
		if(mysqli_num_rows( $open_intro_res ) > 0 ) {
			...
			<table style="width:100%;" border="1">
				<tbody>
					<tr>
						<th colspan="4" bgcolor="#75CCE0" style="text-align: center;"><strong><span class="list-title">USDF 2015 Introductory Level Test A</span></strong></th>
					</tr>
				</tbody>
			</table>
			<table style="width:100%;" border="1">
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name FROM  
			( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) 
			INNER JOIN horses ON entries.HorseID = horses.ID ) INNER JOIN classes ON entries.ID = classes.EntryID ) 
			INNER JOIN tests ON classes.TestID = tests.ID WHERE entries.ShowID = 6 
			AND entries.Status <> 'Scratch' AND tests.ID = 2 AND riders.Level = 'Open'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
		...
		if(mysqli_num_rows( $open_intro_res ) > 0 ) {
			...
			<table style="width:100%;" border="1">
				<tbody>
					<tr>
						<th colspan="4" bgcolor="#75CCE0" style="text-align: center;"><strong><span class="list-title">USDF 2015 Introductory Level Test B</span></strong></th>
					</tr>
				</tbody>
			</table>
			<table style="width:100%;" border="1">
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name FROM  
			( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) 
			INNER JOIN horses ON entries.HorseID = horses.ID ) INNER JOIN classes ON entries.ID = classes.EntryID ) 
			INNER JOIN tests ON classes.TestID = tests.ID WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 3 AND riders.Level = 'Open'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
		...
		if(mysqli_num_rows( $open_intro_res ) > 0 ) {
			...
			?>
				...
				<tbody>
					<tr>
						<th colspan="4" bgcolor="#75CCE0" style="text-align: center;"><strong><span class="list-title">USDF 2015 Introductory Level Test C</span></strong></th>
					</tr>
				</tbody>
			</table>
			<table style="width:100%;" border="1">
...
<?php
		...
		$intro_entries_sql = "SELECT entries.ID, riders.ID, classes.ID, tests.ID 
				FROM ( ( entries INNER JOIN riders ON riders.ID = entries.RiderID ) 
				INNER JOIN classes ON classes.EntryID = entries.ID ) INNER JOIN tests 
				ON tests.ID = classes.TestID WHERE tests.ID IN (1, 2, 3) AND riders.Level = 'Amateur' AND entries.ShowID = 6";
		$intro_entries_res = mysqli_query( $mysqli, $intro_entries_sql ) or die(mysqli_error( $mysqli ) );
	...
	<table id="table-wrap" style="width:100%;" border="1">
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name 
			...
			WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 1 AND riders.Level = 'Amateur'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name FROM  
			( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) 
			INNER JOIN horses ON entries.HorseID = horses.ID ) INNER JOIN classes ON entries.ID = classes.EntryID ) 
			INNER JOIN tests ON classes.TestID = tests.ID WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 2 AND riders.Level = 'Amateur'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name FROM  
			( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) 
			INNER JOIN horses ON entries.HorseID = horses.ID ) INNER JOIN classes ON entries.ID = classes.EntryID ) 
			INNER JOIN tests ON classes.TestID = tests.ID WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 3 AND riders.Level = 'Amateur'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
...
<?php
		...
		$intro_entries_sql = "SELECT entries.ID, riders.ID, classes.ID, tests.ID 
				FROM ( ( entries INNER JOIN riders ON riders.ID = entries.RiderID ) 
				INNER JOIN classes ON classes.EntryID = entries.ID ) INNER JOIN tests 
				ON tests.ID = classes.TestID WHERE tests.ID IN (1, 2, 3) AND riders.Level = 'Junior' AND entries.ShowID = 6";
		$intro_entries_res = mysqli_query( $mysqli, $intro_entries_sql ) or die(mysqli_error( $mysqli ) );
	...
	<table id="table-wrap" style="width:100%;" border="1">
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name 
			FROM  ( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) INNER JOIN horses ON entries.HorseID = horses.ID ) 
			INNER JOIN classes ON entries.ID = classes.EntryID ) INNER JOIN tests ON classes.TestID = tests.ID 
			WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 1 AND riders.Level = 'Junior'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.name FROM  
			( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) 
			INNER JOIN horses ON entries.HorseID = horses.ID ) INNER JOIN classes ON entries.ID = classes.EntryID ) 
			INNER JOIN tests ON classes.TestID = tests.ID WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 2 AND riders.Level = 'Junior'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
		...
		$open_intro_sql = "SELECT riders.FirstName, riders.LastName, horses.Name, classes.ID, tests.ID FROM  
			( ( ( entries INNER JOIN riders on entries.riderID = riders.ID ) 
			INNER JOIN horses ON entries.HorseID = horses.ID ) INNER JOIN classes ON entries.ID = classes.EntryID ) 
			INNER JOIN tests ON classes.TestID = tests.ID WHERE entries.ShowID = 6 AND entries.Status <> 'Scratch' AND tests.ID = 3 AND riders.Level = 'Junior'";
		$open_intro_res = mysqli_query( $mysqli, $open_intro_sql ) or die(mysqli_error( $mysqli ) );
