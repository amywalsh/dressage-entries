...
<form>
			...
			<select id="entry" name="entryID" onchange="showTests(this.value)">
				<option value="">SELECT Horse/Rider COMBINATION</option>
				<?php

				$get_entries_sql = 'SELECT entries.ID, entries.EntryNumber, riders.LastName, riders.FirstName, horses.Name FROM ( ( entries INNER JOIN riders ON entries.RiderID = riders.ID ) INNER JOIN horses ON entries.HorseID = horses.ID ) WHERE entries.ShowID = ' . $e . ' AND entries.Status <> "Scratch" ORDER BY entries.EntryNumber ASC';
